    ...
    for down in range(zwidth-1):
        ...
        if 1 == 2:
            ...
            if not os.path.exists(os.path.join(str(usepath), str(suffix))):
                os.mkdir(os.path.join(str(usepath), str(suffix)))
        ...
        with open(strpath,'w') as f:            
            ...
            print("MAKING CSV:",usepath, suffix)
            df.to_csv(os.path.join(str(usepath),str(suffix),'dungeon-stats.csv'), index=False)
...
if __name__ == '__main__':
    ...
    #with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        #future_to_data = {executor.map(dungeon_sim, idx, usepath_list[idx], pc_list[idx], verbosity_list[idx],  rooms_check_list[idx], levels_check_list[idx],chunksize=10_000): idx for idx in idx_list}		
    ...
    output.to_csv(os.path.join(str(usepath),'dungeon-stats_simulation.csv'), index=False)
